...
function localGeometry(c::config,x,y,z)
	...
	for dir = 1:3
		...
		edges+=new_edge(c,shifted_1[1],shifted_1[2],shifted_1[3],dir);
		edges+=new_edge(c,shifted_2[1],shifted_2[2],shifted_2[3],dir);
		edges+=new_edge(c,shifted_both[1],shifted_both[2],shifted_both[3],dir);
	end
...
function new_localGeometry(c::config,x,y,z)
	...
	for dir = 1:3
		...
		for n=1:4
			...
			# the middle one
			edges += new_edge(c,allaxes[n][1],allaxes[n][2],allaxes[n][3],dir);

			# the ones off the edge
			shift_plus = shiftAdd(allaxes[n][1],allaxes[n][2],allaxes[n][3],dir,c.NX,c.NY,c.NZ);
			edges += new_edge(c,shift_plus[1],shift_plus[2],shift_plus[3],dir)

			shift_minus = shiftSubtract(allaxes[n][1],allaxes[n][2],allaxes[n][3],dir,c.NX,c.NY,c.NZ);
			edges += new_edge(c,shift_minus[1],shift_minus[2],shift_minus[3],dir)
		end
...
function mcRunEfficient(c::config,beta,phi,Nsteps,sample_rate)
	...
	#chiarray = zeros(numSamples);
	chiArray = zeros(numSamples);
	clustersArray = zeros(numSamples);
	...
	chiArray[1] =chi(c)/sites;
	clustersArray[1] = numClusters(c);
    ...
    for i=1:Nsteps
        ...
        if(mod(i,sample_rate)==0)
			...
			# recomputing chi (since I don't use that!)
			chiArray[whichSample] = chi(c)/sites;
			clustersArray[whichSample]=numClusters(c);
		...
		if (mod(i,2000)==0)
			println("Currently at ", i/Nsteps)
		end
    end


	# return the full arrays. don't do any computations here.
    return (Marray,Earray,sMarray,o3array,o4array,convert(Float32,rejected_steps/Nsteps),chiArray,clustersArray);
end
...
# this just returns the number of clusters alone and not all the other data.
function numClusters(c::config)
	return length(findClusters(c)[2]);
end
...
function phiScan(beta,phi_i,phi_f,nphis,sizeLat,Nsweeps)
	...
	o4s = zeros(nphis);
	chis = zeros(nphis);
	clusterNums = zeros(nphis);
	rejectionRates = zeros(nphis);

	to_store = zeros(nphis, 10);
	...
	@time for i=1:length(phis);
		...
		@time (Marray,Earray,sMarray,o3array,o4array,rejectionRate,chiArray,clusterArray) = mcRunEfficient(s,beta,phis[i],Nsteps,sample_rate);


		Ms[i] = mean(Marray);
		Es[i] = mean(Earray);
		sMs[i] = mean(sMarray);
		chis[i] = mean(chiArray);
		o3s[i] = mean(o3array);
		o4s[i] = mean(o4array);
		clusterNums[i]=mean(clusterArray);
        ...
        writedlm(string(filename, "-chiarray.txt"),chiArray);
		writedlm(string(filename, "-clusterarray.txt"),clusterArray);

		# this is the format in which we output everything
		to_store[i,:]=[beta phis[i] Ms[i] sMs[i] o3s[i] o4s[i] Es[i] rejectionRates[i] chis[i] clusterNums[i]];
